...
function SchemeDetailContent() {
  ...
  useEffect(() => {
    async function fetchScheme() {
      ...
      try {
        // Since backend doesn't support GET /api/schemes/{id}, we fetch all and filter
        const data = await postToBackend<{ schemes: any[] }>('/api/schemes', {
          query: '',
          language: language
        })
        const found = data.schemes.find((s: any) => s.id === schemeId)
  ...
  const handleCheckEligibility = async () => {
    ...
    try {
      const data = await postToBackend<any>('/api/schemes/check-eligibility', {
        scheme_id: schemeId,
        age: parseInt(eligibilityData.age) || 0,
        income: parseInt(eligibilityData.income) || 0,
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {scheme.documents && scheme.documents.length > 0 && (
              <Card>
                ...
                <CardContent>
                  <ul className="space-y-2">
                    {scheme.documents.map((item: string, index: number) => (
                      <li key={index} className="flex items-start gap-2">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          fill="none"
            ...
            {scheme.limitations && scheme.limitations.length > 0 && (
              <Card className="border-2 border-[#F59E0B]">
                ...
                <CardContent>
                  <ul className="space-y-2">
                    {scheme.limitations.map((item: string, index: number) => (
                      <li key={index} className="flex items-start gap-2">
                        <svg
                          xmlns="http://www.w3.org/2000/svg"
                          fill="none"
            ...
            {scheme.rejectionReasons && scheme.rejectionReasons.length > 0 && (
              <Card className="border-2 border-[#DC2626]">
                ...
                <CardContent>
                  <ul className="space-y-2">
                    {scheme.rejectionReasons.map((item: string, index: number) => (
                      <li key={index} className="flex items-start gap-2">
              ...
              <Card>
                ...
                <CardContent>
                  <ol className="space-y-3">
                    {scheme.steps.map((step: string, index: number) => (
          ...
          <div className="lg:col-span-1">
            <Card className="sticky top-20 border-2 border-primary">
              ...
              <CardContent className="space-y-4">
                ...
                <Button onClick={handleCheckEligibility} className="w-full h-12" disabled={isChecking}>
